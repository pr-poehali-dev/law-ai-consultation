...
Ответ: до 400 слов, чётко и по делу."""

# ───────────────────────────────────────────────
# СИСТЕМНЫЙ ПРОМПТ — ГЕНЕРАЦИЯ ДОКУМЕНТА (сжатый)
# ───────────────────────────────────────────────
...
БЛОКИ (каждый начинай с маркера на отдельной строке):
...
[ПРИМЕЧАНИЯ] — что вписать вручную, сроки

СПЕЦИФИКА:
- Иски (ст.131–132 ГПК): цена иска прописью, госпошлина (ст.333.19 НК), подсудность (ст.28 ГПК)
- Договоры (ст.432 ГК): предмет, цена, сроки, неустойка (ст.330), расторжение (ст.450–453), форс-мажор (ст.401)
- Претензии: срок ответа (ст.165.1 ГК)
- Жалобы: срок обжалования (ст.30.3 КоАП / ст.256 КАС)

РЕКВИЗИТЫ: известные данные вставляй напрямую. Неизвестные — только метки {{{{ПОЛЕ_НАЗВАНИЕ}}}} (русский язык, подчёркивание). Запрещены [...] и ___.
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if mode == "doc_generate":
            ...
            answer = call_yandex(SYSTEM_DOC_GENERATE, [{"role": "user", "content": prompt}], max_tokens=1800)
        ...
        # ── Обычный чат-консультация ──
            ...
            answer = call_yandex(SYSTEM_CHAT, messages, max_tokens=1200)
            return {"statusCode": 200, "headers": {**CORS, "Content-Type": "application/json"},
