...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if mode == "doc_recommendations":
            ...
            # Ищем по сути документа (первые символы содержания), а не только по названию типа —
            # иначе поиск находит практику, совпадающую по общим словам («иск», «взыскание»),
            # но нерелевантную конкретному делу
            legal_ctx = ""
            try:
                query_basis = f"{doc_name} {doc_content[:300]}"
                legal_ctx = get_legal_context_for_ai("case_law", max_files=2, max_chars=2000, query=query_basis)
            except Exception:
                pass
            # Обязательные системные рекомендации — добавляются программно (не AI),
            # чтобы гарантированно присутствовать в каждом документе с кнопками действий.
            mandatory_recs = [{
                "type": "case_law_check",
                "title": "Проверьте судебную практику",
                "reason": "Внимание! Обязательно проверяйте судебную практику по документу с использованием функции «Судебная практика» в разделе «Чат с AI» — это поможет убедиться, что позиция в документе актуальна и подкреплена реальными делами.",
            }]
            has_duty_mention = bool(re.search(r"госпошлин", doc_content, re.IGNORECASE))
            if has_duty_mention:
                mandatory_recs.append({
                    "type": "duty_check",
                    "title": "Перепроверьте госпошлину",
                    "reason": "Обязательно перепроверяйте расчёт госпошлины через инструмент «Госпошлина» на сервисе — ошибка в расчёте может привести к оставлению иска без движения.",
                })
            ...
            rec_pr = (
                # doc_content уже приходит с фронтенда с разумным лимитом (начало+конец для длинных
                # документов) — здесь НЕ обрезаем повторно, иначе теряется просительная часть/подпись в конце
                f"Документ «{doc_name}»:\n{doc_content[:4200]}\n\n"
                + (f"Актуальная судебная практика по теме (из нашей базы):\n{legal_ctx}\n\n" if legal_ctx else "")
                ...
                '2. {"type": "state_duty", "title": "Госпошлина", "reason": "...", "duty_note": "Замечание"} — ТОЛЬКО если в документе явно неверно РАССЧИТАНА госпошлина (конкретная ошибка в сумме)\n'
                ...
                "— state_duty — только при реальной конкретной ошибке в сумме госпошлины (не общее напоминание перепроверить — это уже есть отдельно)\n"
                "— penalty_calc и doc — только при реальной необходимости\n"
                "— ЗАПРЕЩЕНО создавать general-рекомендацию с советом «проверьте/добавьте судебную практику» или "
                "«перепроверьте госпошлину через инструмент» — эти напоминания уже добавлены системой отдельно, не дублируй их\n"
                "— ПРО СУДЕБНУЮ ПРАКТИКУ В ТЕКСТЕ: если в документе НЕТ упоминаний судебной практики — "
                "НЕ советуй «заменить примеры практики» или «сослаться на практику», если её и так нет в тексте. "
                "Никогда не выдумывай номера обзоров ВС РФ, дел или конкретных решений — "
                "ссылайся только на практику из блока выше (если он есть) или на реальные, точно известные Пленумы ВС РФ.\n"
            ...
            final_recs = mandatory_recs + recs_raw
            return {"statusCode": 200, "headers": {**CORS, "Content-Type": "application/json"},
                    "body": json.dumps({"recommendations": final_recs}, ensure_ascii=False)}
