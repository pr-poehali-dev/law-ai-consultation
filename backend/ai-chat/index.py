...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if mode == "case_law_assessment":
            ...
            assessment_system = (
                "Ты — Правосудик, юрист с 15-летней практикой. Оцениваешь перспективы дела клиента "
        ...
        if mode == "doc_hint_suggest":
            ...
            hint_system = (
                ...
                "ОБЯЗАТЕЛЬНО начни ответ ровно с фразы «В вашей ситуации советую» — и сразу продолжи "
                "точным названием документа, который нужен по ситуации. "
            ...
            # Гарантия фиксированного вступления — если модель не начала с нужной фразы,
            # подставляем её программно (регистронезависимая проверка на всякий случай).
            if hint_answer and not hint_answer.lower().startswith("в вашей ситуации советую"):
                hint_answer = "В вашей ситуации советую " + hint_answer[0].lower() + hint_answer[1:]
            return {"statusCode": 200, "headers": {**CORS, "Content-Type": "application/json"},
