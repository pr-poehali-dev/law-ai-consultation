...
def resp(status, body, extra_headers=None):
    ...
    return {"statusCode": status, "headers": headers, "body": json.dumps(body, default=json_serial)}
...
def handler(event: dict, context) -> dict:
    """Органайзер: CRUD для дел, заседаний, задач и документов."""
    ...
    try:
        ...
        if path == "/cases" or path == "":
            ...
            if method == "POST":
                ...
                court = body.get("court", "").strip()
                if not case_number or not court:
                    return resp(400, {"error": "case_number и court обязательны"})
                with conn.cursor() as cur:
                    cur.execute(
                        f"""INSERT INTO {SCHEMA}.organizer_cases (user_id, case_number, court, judge, plaintiff, defendant, status)
                        VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id""",
                        (user_id, case_number, court,
                         body.get("judge"), body.get("plaintiff"), body.get("defendant"),
        ...
        if path.startswith("/cases/"):
            parts = path.split("/")
            case_id = int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else None
            sub = parts[3] if len(parts) > 3 else None
            sub_id = int(parts[4]) if len(parts) > 4 and parts[4].isdigit() else None

            if not case_id:
                return resp(400, {"error": "invalid case_id"})

            # Verify ownership
            with conn.cursor() as cur:
                cur.execute(f"SELECT id FROM {SCHEMA}.organizer_cases WHERE id=%s AND user_id=%s", (case_id, user_id))
                if not cur.fetchone():
            ...
            if sub is None and method == "PUT":
                with conn.cursor() as cur:
                    cur.execute(
                        f"""UPDATE {SCHEMA}.organizer_cases SET case_number=%s, court=%s, judge=%s,
                        plaintiff=%s, defendant=%s, status=%s, updated_at=NOW()
                        WHERE id=%s AND user_id=%s""",
                        (body.get("case_number"), body.get("court"), body.get("judge"),
                         body.get("plaintiff"), body.get("defendant"), body.get("status", "active"),
                         case_id, user_id)
                    )
                conn.commit()
                return resp(200, {"ok": True})

            # Delete case (soft: mark closed; actual rows kept for now)
            ...
            # Get full case with hearings/tasks/docs
            if sub is None and method == "GET":
                with conn.cursor() as cur:
                    cur.execute(f"SELECT id,case_number,court,judge,plaintiff,defendant,status,created_at FROM {SCHEMA}.organizer_cases WHERE id=%s", (case_id,))
                    c = dict(zip([d[0] for d in cur.description], cur.fetchone()))
                    cur.execute(f"SELECT id,hear_date,hear_time,room,result,notes FROM {SCHEMA}.organizer_hearings WHERE case_id=%s ORDER BY hear_date", (case_id,))
                    c["hearings"] = [dict(zip([d[0] for d in cur.description], r)) for r in cur.fetchall()]
                    cur.execute(f"SELECT id,title,due_date,is_completed,reminder FROM {SCHEMA}.organizer_tasks WHERE case_id=%s ORDER BY due_date NULLS LAST", (case_id,))
            ...
            if sub == "hearings":
                if method == "POST":
                    ...
                    with conn.cursor() as cur:
                        cur.execute(
                            f"""INSERT INTO {SCHEMA}.organizer_hearings (case_id,user_id,hear_date,hear_time,room,notes)
                    ...
                    conn.commit()
                    return resp(201, {"id": new_id})
                if method == "PUT" and sub_id:
                    with conn.cursor() as cur:
                        cur.execute(
                            f"UPDATE {SCHEMA}.organizer_hearings SET hear_date=%s,hear_time=%s,room=%s,notes=%s,result=%s WHERE id=%s AND user_id=%s",
                            (body.get("hear_date"), body.get("hear_time"), body.get("room"), body.get("notes"), body.get("result"), sub_id, user_id)
                        )
                    conn.commit()
                    return resp(200, {"ok": True})
                if method == "DELETE" and sub_id:
                    with conn.cursor() as cur:
                        cur.execute(f"UPDATE {SCHEMA}.organizer_hearings SET result='deleted' WHERE id=%s AND user_id=%s", (sub_id, user_id))
                    conn.commit()
                    return resp(200, {"ok": True})
            ...
            if sub == "tasks":
                if method == "POST":
                    ...
                    with conn.cursor() as cur:
                        cur.execute(
                            f"""INSERT INTO {SCHEMA}.organizer_tasks (case_id,user_id,title,due_date,reminder)
                            VALUES (%s,%s,%s,%s,%s) RETURNING id""",
                            (case_id, user_id, title, body.get("due_date") or None, body.get("reminder", True))
                        )
                        new_id = cur.fetchone()[0]
                    conn.commit()
                    return resp(201, {"id": new_id})
                if method == "PUT" and sub_id:
                    with conn.cursor() as cur:
                        cur.execute(
                            ...
                            (body.get("title"), body.get("due_date") or None, body.get("is_completed", False), body.get("reminder", True), sub_id, user_id)
                        )
                    conn.commit()
                    return resp(200, {"ok": True})
                if method == "DELETE" and sub_id:
                    with conn.cursor() as cur:
                        cur.execute(f"UPDATE {SCHEMA}.organizer_tasks SET is_completed=TRUE WHERE id=%s AND user_id=%s", (sub_id, user_id))
                    conn.commit()
                    return resp(200, {"ok": True})
            ...
            if sub == "documents":
                if method == "POST":
                    ...
                    with conn.cursor() as cur:
                        cur.execute(
                            f"""INSERT INTO {SCHEMA}.organizer_documents (case_id,user_id,name,doc_type,deadline,notes)
                            VALUES (%s,%s,%s,%s,%s,%s) RETURNING id""",
                            (case_id, user_id, name, body.get("doc_type", "Другое"), body.get("deadline") or None, body.get("notes"))
                        )
                        new_id = cur.fetchone()[0]
                    conn.commit()
                    return resp(201, {"id": new_id})
                if method == "PUT" and sub_id:
                    with conn.cursor() as cur:
                        cur.execute(
                            ...
                            (body.get("name"), body.get("doc_type", "Другое"), body.get("is_prepared", False), body.get("deadline") or None, body.get("notes"), sub_id, user_id)
                        )
                    conn.commit()
                    return resp(200, {"ok": True})
                if method == "DELETE" and sub_id:
                    with conn.cursor() as cur:
                        cur.execute(f"UPDATE {SCHEMA}.organizer_documents SET is_prepared=TRUE WHERE id=%s AND user_id=%s", (sub_id, user_id))
                    conn.commit()
                    return resp(200, {"ok": True})
