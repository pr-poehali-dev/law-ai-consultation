  ...
  return (
    ...
    >
      ...
      <div className={`max-w-[80%] sm:max-w-[72%] min-w-0 flex flex-col gap-1 ${isMe ? "items-end" : "items-start"}`}>
        ...
        {/* Пузырь */}
        <div className={`rounded-2xl px-4 py-3 shadow-sm flex flex-col gap-2 min-w-0 w-full ${
          isMe
        ...
        <div className={`flex items-center gap-1.5 ${isMe ? "flex-row-reverse" : ""}`}>
          ...
          {isMe && (
            isOptimistic
              ? <Icon name="Clock" size={11} className="text-muted-foreground/40" title="Отправляется..." />
              : msg.is_read
                ? <Icon name="CheckCheck" size={12} className="text-gold-400" title="Прочитано" />
                : <Icon name="Check" size={12} className="text-muted-foreground/50" title="Доставлено" />
          )}
        </div>
      </div>
