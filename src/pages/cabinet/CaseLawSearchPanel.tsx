  ...
  const search = async () => {
    ...
    if (isCodex) {
      ...
      fetch(LEGAL_DOCS_URL, {
        ...
        body: JSON.stringify({ sub: "search", query: q, category: "codex", limit: 10 }),
      })
        .then(r => r.json())
        .then(data => {
        ...
        {searched && isCodex && (
          <div className="rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2"
              ...
              <p className="text-[10px] font-bold text-slate-700 flex-1">База кодексов и законов</p>
            ...
            {dbLoading && (
              <div className="px-3 py-3 space-y-1.5">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-[11px] text-slate-400">Ищу статьи в базе законодательства...</p>
                </div>
                {[70, 90, 55].map((w, i) => (
            ...
            {!dbLoading && dbResults && dbResults.length === 0 && !dbError && (
              <div className="px-3 py-3 text-center">
                <p className="text-[11px] text-slate-400">В базе кодексов ничего не найдено</p>
                <p className="text-[10px] text-slate-300 mt-1">Попробуйте: «статья 333 ГК РФ», «ст. 14 ТК РФ»</p>
              </div>
            )}
            ...
            {isCodex ? (
              <>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-3"
                  ...
                  <Icon name="BookMarked" size={18} color="#0f4c81" />
                </div>
