...
export function useChatLogic({ refreshUser, onPaymentRequired }: UseChatLogicProps) {
  ...
  const sendMessage = async (overrideText?: string) => {
    ...
    // Детектируем конкретный запрос расчёта неустойки — предлагаем калькулятор кнопкой
    const penaltyKeywords = /^(рассчитай|посчитай|подсчитай|сделай\s+расчёт|сделай\s+расчет|рассчитать|посчитать)\s+(неустойку|пени|пеню|штраф|неустойки|пеней)/i;
    if (penaltyKeywords.test(userMsg.trim())) {
      setMessages((p) => [
        ...p,
