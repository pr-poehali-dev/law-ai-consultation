    ...
    <>
      ...
      <div className={`flex items-center gap-3 px-4 py-3 shrink-0 border-b border-slate-200 ${justUpdated ? "bg-emerald-50" : "bg-white"}`}>
        <div className="relative shrink-0">
          ...
          {editLoading && (
            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white bg-gold-400 animate-ping" />
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {historyCount > 0 && (
              <span className="px-1.5 py-0.5 rounded-md text-[9px] font-semibold bg-slate-100 text-slate-500 border border-slate-200">
          ...
          <p className="text-[10px] text-slate-500 truncate">
            {justUpdated
              ? `Правка #${editCount} — изменения подсвечены в документе`
              : analyzing
                ? "Анализирую документ..."
                : docName}
          </p>
      ...
      <div className="flex items-center justify-between px-4 py-1.5 shrink-0 border-b border-slate-100 bg-slate-50">
        <p className="text-[10px] text-slate-500">
          1 правка = 5 вопросов · документы не списываются · сохраняется в браузере
        </p>
        {editCount > 0 && (
